...
import com.nadeveloper.nola.Model.Cuisine;
import com.nadeveloper.nola.Model.CuisineSearch;
import com.nadeveloper.nola.Model.Cuisine_;
...
public class CuisinePreference extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener, Button.OnClickListener {
    ...
    private void setCuisineListView(List<Cuisine> cuisineList) {
        ...
        String[] selectedCuisineIdArray = selectedCuisineIdString.split(getResources().getString(R.string.commaOne));
        ...
        if(isNetworkAvailable()) {
            ...
            restaurantAPIService.getCuisines(latitude, longitude)
                    .enqueue(new Callback<CuisineSearch>() {
                        @Override
                        public void onResponse(@NonNull Call<CuisineSearch> call, @NonNull Response<CuisineSearch> response) {
                        ...
                        @Override
                        public void onFailure(@NonNull Call<CuisineSearch> call, @NonNull Throwable t) {
                            cuisineRefreshLayout.setRefreshing(false);
